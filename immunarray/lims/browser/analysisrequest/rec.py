...
class AddRecView(BrowserView):
    ...
    def __call__(self):
        ...
        if "usn_update" in request.form:
            ...
            if check_if_site_is_sales_rep is True:
                return json.dumps({"site_name": site_name,
        ...
        if "all_data" in request.form:
            ...
            if usn_from_form == 'undefined-undefined':
                ...
                if pt_UID != "new_patient":
                    print (
                    "PT UID: " + pt_UID + " USN from Form : " + usn_from_form)
                    # update existing record
                    self.update_existing_patient_data(pt_UID, dob, first, last,
                                                      mrn, ssn, gender,
                    ...
                    self.make_patient(first, last, ssn, mrn, dob, gender,
                                      ethnicity,
                                      ethnicity_other, marital_status,
                                      patient_address,
                                      patient_city, patient_state,
                                      patient_zip_code,
                                      patient_phone, usn_from_form)
                sample_UID = self.make_clinical_sample(usn_from_form,
                                                       consent_acquired,
                                                       ana_testing, clin_rash,
                                                       clin_seiz_psych,
                                                       clin_mouth_sores,
                                                       clin_hair_loss,
                                                       clin_joint_pain,
                                                       clin_inflam, clin_other,
                                                       clin_other_specify,
                                                       diag_D89_89, diag_M32_10,
                                                       diag_D89_9,
                                                       diag_M35_9, diag_L93_2,
                                                       diag_other,
                                                       diag_other_specify,
                                                       provider_nip_clean,
                                                       provider_signed,
                                                       draw_location, draw_tel,
                                                       phlebotomist_name,
                                                       draw_signed,
                                                       collection_date,
                                                       shipment_date,
                ...
                # easy way
                draw_tubes = ['A', 'B']
                bulk_aliquotA = self.make_bulk_aliquots(sample_UID,
                                                        usn_from_form,
                                                        draw_tubes[0])
                bulk_aliquotB = self.make_bulk_aliquots(sample_UID,
                                                        usn_from_form,
                                                        draw_tubes[1])
                # make working aliquots
                working_tubes = ['02', '03', '04']
                working_aliquotA02 = self.make_working_aliquots(usn_from_form,
                                                                bulk_aliquotA,
                                                                working_tubes[
                                                                    0])
                working_aliquotA03 = self.make_working_aliquots(usn_from_form,
                                                                bulk_aliquotA,
                                                                working_tubes[
                                                                    1])
                working_aliquotA04 = self.make_working_aliquots(usn_from_form,
                                                                bulk_aliquotA,
                                                                working_tubes[
                                                                    2])
    ...
    def providers_at_site(self, site_id):
        ...
        for i in provider_uids:
            ...
            if int(site_id) == provider_object.site_ID:
                element = provider_object.first_name + " " + provider_object.last_name + "-" + str(
                    provider_object.npi)
    ...
    def repeat_order_check(self, dob_string, patient_first_name,
        ...
        for c in current_pt_list:
            if patient_first_name == c[0] and patient_last_name == c[
                1] and dob_string == c[2]:
    ...
    def make_clinical_sample(self, usn_from_form, consent_acquired, ana_testing,
                             clin_rash,
                             clin_seiz_psych, clin_mouth_sores, clin_hair_loss,
                             clin_joint_pain,
                             clin_inflam, clin_other, clin_other_specify,
                             diag_D89_89, diag_M32_10, diag_D89_9,
                             diag_M35_9, diag_L93_2, diag_other,
                             diag_other_specify,
                             provider_nip_clean, provider_signed, draw_location,
                             draw_tel, phlebotomist_name,
                             draw_signed, collection_date, shipment_date,
                             test_other_specify,
                             clinical_impression, ordering_provider_name,
                             site_id, assay_selection, other_clinical_symptoms,
                             other_inflam, other_joint_pain):
        ...
        try:
            py_collection_date = datetime.datetime.strptime(collection_date,
                                                            "%Y-%m-%d").date()
        except:
            py_collection_date = None
        try:
            py_shipment_date = datetime.datetime.strptime(shipment_date,
                                                          "%Y-%m-%d").date()
        except:
    ...
    def update_existing_patient_data(self, pt_UID, dob, first, last, mrn, ssn,
                                     gender, marital_status, ethnicity,
                                     ethnicity_other,
                                     patient_address, patient_city,
                                     patient_state, patient_zip_code,
                                     patient_phone):
